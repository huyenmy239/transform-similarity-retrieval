...
def add_cost_function():
    ...
    ttk.Button(frame, text="Xác Nhận", command=submit, style="Accent.TButton").pack(pady=20)
...
def evaluate_cost():
    ...
    ttk.Button(frame, text="Tính Chi Phí", command=submit, style="Accent.TButton").pack(pady=20)
    ...
    try:
        with open(server.path, "r") as f:
            ...
            for func in data:
                ...
                listbox.insert(tk.END, f"{func['name']} ({func['type']}): {display_text}")
    except:
        pass
...
ttk.Button(button_frame, text="➕ Thêm Hàm Chi Phí", command=add_cost_function, style="Accent.TButton").pack(side="left", padx=5)
ttk.Button(button_frame, text="🧮 Tính Chi Phí", command=evaluate_cost, style="Accent.TButton").pack(side="left", padx=5)

ttk.Label(main_frame, text="📄 Danh Sách Hàm Chi Phí", font=("Arial", 12, "bold")).pack(anchor="w", pady=10)

scrollbar = ttk.Scrollbar(main_frame)
scrollbar.pack(side="right", fill="y")

listbox = tk.Listbox(main_frame, width=80, height=15, font=("Arial", 10), yscrollcommand=scrollbar.set)
